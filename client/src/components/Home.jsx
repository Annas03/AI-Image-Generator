...
const Home = () => {
  ...
  return (
    <div className='bg-[#f9fafe] h-screen border border-t-slate-200'>
        ...
        <FormField/>
        {!imgList ? <img src={loader} className="w-8 sm:w-10 md:w-14 xl:w-20 mx-auto mt-8"/>:<div className='pt-8 w-11/12 mx-auto grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 max-w-7xl grid-cols-1'>
          {imgList.map((img) => <ShowCase key={img._id} _id={img._id} photo={img.photo} name={img.name}/>)}
        </div>}
    </div>
  )
}
